...
if [ -z "$ip" ]; then
  ip="172.20.10.2"; // my public ip, don't know where this comes from || could use ip=`wget icanhazip.com -qO- | head -n 1`
                        // alternatively could bounce the request off the api server rather than reply on 3rd party
fi
